...
- Preview modal with three tabs: Preview / Code / Prompt Fragment
- Stack panel polish
...
**Layout:**
- Full-screen overlay with centered modal (max-width: 640px).
...
**There is no pinned preview image outside the tab area.** The image lives only inside the Preview tab.

**Tab content:**

- **Preview tab:** Large preview image (16:9, `object-cover`). If the component has multiple variants, a pill-tab row appears above the image listing variant names. Selecting a variant switches the displayed image and the content shown in Code and Prompt tabs.
...
| `src/components/library/PreviewModal.tsx` | New — tabbed preview modal |
...
- [ ] Modal Preview tab shows the component image
