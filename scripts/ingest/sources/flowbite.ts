...
export async function loadFlowbite(root: string, rules: Array<{ pattern: string; category: string }>) {
  const files = await collectFiles(root, [".html", ".tsx"]) 
  ...
  for (const filePath of files) {
    ...
    components.push({
      ...
      category,
      code: content,
      previewHtml: content.includes("<") ? content : undefined,
